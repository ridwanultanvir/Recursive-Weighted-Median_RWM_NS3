...
static  std::ofstream TcpSocketFile1("TcpSocketFile1.txt");
static  std::ofstream TcpSocketFile2("TcpSocketFile2.txt");
...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    //<===============================================================================================>
    // ==========================================AMAR ADD KORA ================================================>
    .AddAttribute ("CheckIfRttWeightedMedian",
               "CheckIfRttWeightedMedian",
               UintegerValue (1),
               MakeUintegerAccessor (&TcpSocketBase::CheckIfRttWeightedMedian),
               MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("mu_paper",
                   "mu_paper range [4.3, 4.7] ideally 4.5",
                   DoubleValue (4.5),
                   MakeDoubleAccessor (&TcpSocketBase::mu_paper),
                   MakeDoubleChecker<double> (4.3,4.5))
    //<===============================================================================================>
    // UPORER 2ta ATTRIBUTE AMAR nijer ADD KORA 
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  if(CheckIfRttWeightedMedian==mydefinedrttweightedmedianvalue){
    ...
    if(lambda_paper<0){
      TcpSocketFile1<<"lambda_paper<0 found\n"; 
    }
    // double oldRttVar = m_estimatedVariation.ToDouble(Time::S);
    double last_estimated_rtt1 = m_rtt->GetEstimate ().ToDouble(Time::S);

    if(last_estimated_rtt1<0){
      TcpSocketFile1<<"last_estimated_rtt1<0 found\n"; 
    }
    // //basically last RTT JETA AMI ESTIMATE korsi 
    double paper_rto = lambda_paper*last_estimated_rtt1; 
    // if(paper_rto<0){
    //   TcpSocketFile1<<"paper_rto<0 found\n"; 
    // }

    /*
    khub important; 
    m_rto KINTU TIME type er variable

    directly double likhe dile error hobe 
    ...
    */ 
    ...
    //mane abar jate m_minRto er cheye kom NA Hoy 
    // .AddAttribute ("MinRto",TimeValue (Seconds (1.0)), // RFC 6298 says min RTO=1 sec, but Linux uses 200ms.
    // already m_minRto defined 
