...
public:
  ...
  std::vector<double> GetVectorRttEstimate(void) const; 
  std::vector<double> GetVectorRttActual(void) const; 
private:
...
protected:
  ...
  // m_estimatedRtt etar motoi 
  // just ekta vector e ami AGER SHOB DATA rekhe dite ditesi
  // eta future e help korbe 
  std::vector<double> m_estimated_rtt_vector;
  std::vector<double> m_actual_rtt_vector;
};
 ...
 *
 */
class RttMyWeightedMedian : public RttEstimator {
public:
  ...
  // Copy constructor
  RttMyWeightedMedian (const RttMyWeightedMedian& r);
...
private:
  ...
  */
  ...
  int32_t m_observedkoyta_M; // paper er "M" variable ; M = 5 
  ...
  // AMAR DATA GATHERING 
  //eta 1. MEAN ERROR BER Kora
  //2. new RTT; 3. New RTO te kaje lagbe 
  std::vector<double> m_estimated_rtt_vector;
  std::vector<double> m_actual_rtt_vector;
